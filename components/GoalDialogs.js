...
export function GoalDetail({entry,active,embedded=false}) {
  ...
  const toggle=async(id,done)=>{
    ...
    if(focused?.closest('[data-goal-task]')?.dataset.goalTask===id)focusAfterMove.current={id,control:focused.matches('.check')?'.check':'.goal-task-title'};
    setFailure(null);setMotion(current=>({...current,[id]:done?'done':'undo'}));
    clearTimeout(timers.current.get(id));
    timers.current.set(id,setTimeout(()=>{
      if(done){setMotion(current=>({...current,[id]:'settling'}));timers.current.set(id,setTimeout(()=>{setMotion(current=>{const next={...current};delete next[id];return next;});timers.current.delete(id);},1000));return;}
  ...
  return <div className="goal-dialog goal-inline goal-calendar">
    ...
    <div ref={rowsRef} className={filter==='week'?'goal-week-columns':'goal-all-days'}>{groups.map((d,i)=>{
      ...
      {!tasks.length&&<p className="goal-day-empty">Chưa có task</p>}{[false,true].map(done=>{const group=tasks.filter(t=>!!t.done===done);if(!group.length)return null;return <div key={String(done)} className="goal-task-group"><h4>{done?'Đã xong':'Chưa xong'} · {group.length}</h4>{!group.length&&<p className="goal-day-empty">{done?'Chưa có task hoàn thành':'Không có task'}</p>}{group.map(t=><div className={`goal-task-item task-row ${reduced?'':motion[t.id]==='done'?'task-rainbow':motion[t.id]==='settling'?'task-settle':motion[t.id]==='undo'?'task-unpop':''}${t.done?' is-complete':''}`} data-goal-task={t.id} data-motion={motion[t.id]} key={t.id}>
        <TaskCheck checked={t.done} label={`Hoàn thành ${t.name}`} onChange={done=>toggle(t.id,done)}/><div><button type="button" className="goal-task-title task-name-text" aria-pressed={!!t.done} title={t.done?'Bấm để bỏ hoàn thành':'Bấm để hoàn thành'} aria-label={`${t.done?'Bỏ hoàn thành':'Hoàn thành'} ${t.name}`} onClick={()=>toggle(t.id,!t.done)}>{t.name}</button><small>{t.session||'Chưa chọn buổi'}</small></div><button type="button" className="goal-task-edit" title="Chỉnh sửa task" aria-label={`Sửa ${t.name}`} onClick={()=>p.setEditTask(t)}>⋯</button>
      </div>)}</div>;})}</section>;
    })}</div>
