...
export function createPlannerStore({ request = requestJson, now = Date.now, cache = {} } = {}) {
  ...
  let status = "loading", syncError = "", goalsError = "", syncing = false, migration = { running: false, error: "", report: null };
  ...
  function emit() {
    snapshot = { tasks: rows("tasks"), goals: rows("goals"), status, serverLoaded, syncing, syncError, goalsError, migration,
  ...
  return { getSnapshot: () => snapshot, subscribe: fn => { listeners.add(fn); return () => listeners.delete(fn); },
    ...
    setMigration: patch => { migration = { ...migration, ...patch }; emit(); },
