...
export async function patchTask(id, patch) {
  ...
  const existing = await notion(`/pages/${id}`);
  if (existing.archived || existing.in_trash) throw new ApiError("Task đã bị xóa từ thiết bị khác.", 410);
  if (existing.parent?.database_id?.replaceAll("-", "") !== TASKS_DB_ID.replaceAll("-", "")) throw new ApiError("Task không thuộc planning database.", 404);
  ...
  const goalUids = new Map();
  const goalPageId = (properties.Goal || existing.properties.Goal)?.relation?.[0]?.id;
  if (goalPageId) {
    // Reuse the UID validated for this write. Otherwise read only the linked
    // page, never the full Goals database (including its History).
    if (patch.goalId && properties.Goal?.relation?.[0]?.id === goalPageId) goalUids.set(goalPageId, patch.goalId);
    else {
      try { goalUids.set(goalPageId, goalFromPage(await notion(`/pages/${goalPageId}`)).uid); }
